...
export default function AppConfigModal({
  ...
  // 当选择的 record 改变时，回显数据
  useEffect(() => {
    if (open && selectedRecord) {
      const isGeneral = !selectedRecord.appId;
      form.setFieldsValue({
        isGeneral,
        appId: selectedRecord.appId || undefined,
        allowedFeatures: selectedRecord.allowedFeatures || [],
      });
      setSelectedAppId(selectedRecord.appId || null);
    } else {
      form.resetFields();
  ...
  const handleSave = async (values: any) => {
    ...
    try {
      ...
      await api.patch(`/register-codes/${selectedRecord.id}/config`, {
        appId,
        allowedFeatures,
      });

      message.success('更新激活码应用与权限配置成功！');
      onSuccess();
    } catch (err: any) {
      ...
      {selectedRecord && (
        <div className="bg-slate-50 dark:bg-zinc-950 p-3 rounded-lg border border-slate-200 dark:border-zinc-800 mb-4 text-xs space-y-1.5">
          <div className="flex justify-between">
            <span className="text-slate-400">当前激活码:</span>
            <span className="font-mono font-bold text-slate-800 dark:text-zinc-200">{selectedRecord.code}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">目前所绑应用:</span>
            <span className="font-semibold text-indigo-600 dark:text-indigo-400">
              {selectedRecord.appName || '通用型'}
            </span>
          </div>
        </div>
      )}
