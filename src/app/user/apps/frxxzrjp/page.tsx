...
export default function UserDeviceListPage() {
  ...
  useEffect(() => {
    api.get('/apps')
      .then((res: any) => {
        const currentApp = (res || []).find((a: any) => a.appKey === 'frxxzrjp');
        if (currentApp) {
      ...
      {devices.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {devices.map((dev) => {
            ...
            // 动态决定专属控制台路由
            const devAppName = (dev as any).appName || '';
            const getDashboardPath = () => {
              if (!devAppName) return null;
              if (devAppName.includes('凡人修仙') || devAppName.includes('frxxzrjp')) {
                return '/user/apps/frxxzrjp';
              }
              if (devAppName.includes('天龙') || devAppName.includes('tlbb')) {
                return '/user/apps/game_tlbb';
              }
              return null;
            };
                  ...
                  <div className="flex items-center gap-2">
                    {dashboardPath && (
                      <Button
                        ...
                        className="bg-indigo-600 hover:bg-indigo-550 dark:bg-indigo-600 dark:hover:bg-indigo-550 text-white text-[10px] font-bold h-8 px-3 rounded-xl flex items-center gap-1.5 transition-all active:scale-97 shadow-md cursor-pointer border border-transparent"
                      >
