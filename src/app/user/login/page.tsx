...
import { KeyRound, ShieldAlert, Loader2, User } from 'lucide-react'
...
function UserLoginForm() {
  ...
  const { setAuth, isHydrated } = useUserStore()
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-zinc-950 select-none">
      ...
      <div className="w-full max-w-md p-4 relative z-10">
        ...
        <div className="flex flex-col items-center gap-3 mb-8">
          <img
            src="/icons/icon-192x192.png"
            className="w-11 h-11 rounded-lg object-contain shadow-lg shadow-emerald-500/10"
            alt="小宝修仙 Logo"
          />
          ...
          <CardContent className="space-y-4">
            ...
            >
              <div className="space-y-1.5">
                <Label
                  htmlFor="code"
                  className="text-zinc-400 text-xs font-bold tracking-wide"
                >
                  注册激活码 (License Code)
                </Label>
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full bg-emerald-500 hover:bg-emerald-400 text-zinc-950 hover:text-zinc-950 active:scale-98 transition-transform font-black mt-2"
              >
                {loading ? (
                  <>
...
export default function UserLoginPage() {
  return (
    <React.Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center bg-zinc-950 text-zinc-500 text-xs font-mono select-none">
